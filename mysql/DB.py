...
class DB(CORE):
	...
	def Dictionaries(self,Dir):  #批量导入字典方法
		try:
			f = open(Dir,'r')
			sql = "select count(table_name) from information_schema.tables where table_name = 'lis' and TABLE_SCHEMA = '%s'" % (CORE.db);
			if not DB().query(sql):
	...
	def  Domain_storage(self,tables,url,ip): #域名入库
		try:
			...
			if DB().query(sql) == 0:#入库之前 先判断数据库是否含有相同信息 如果没有就入裤
				...
				sql = "insert into %s values(null,\"%s\",\"%s\",0,0)" % (tables,url,ip);
				DB().increase(sql);
		except Exception,e:
